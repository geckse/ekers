...
{
  // Erstmal daf�r sorgen, dass das Objekt passt
  ...
  // Kälte setzen
  if(pObject->~GetTemperature() > -160) pObject->~SetTemperature(-160);
...
global func FxFreezeStart(pTarget, iEffectNumber, iTemp, iFromController)
{
  ...
  // Bei einer tempor�ren �nderung aufh�hren
  if(iTemp) return();
  // Toller Sound
  Sound("Crystal2", 0, pTarget);
  // Zeit einstellen
  EffectVar(0, pTarget, iEffectNumber)+=18;
  // ID speichern
...
{
  // In Lava schwimmend? Dann schnell auftauen (nur f�r Monster und andere nicht brennbare Lebewesen wichtig)
  if(GetMaterialVal("Incindiary", "Material", pTarget->GetMaterial()))
    return -1;
  // Partikeleffekte f�r den betroffenen
  var id = GetID(pTarget);
...
global func FxFreezeStop(pTarget, iEffectNumber, iReason, fTemp)
{
  ...
  // Wenn der Effekt endg�ltig entfernt wird: Sound!
  if(fTemp) return();
